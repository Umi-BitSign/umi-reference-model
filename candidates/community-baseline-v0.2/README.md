...
absent from Git. S1's backend is unchanged. For UMI's separate Linux CPU
`offline_bundle/1` integration, use the [community integration guide](../../community/README.md).
...
`MODEL_IDENTITY.json` retains the supplied assembly metadata. Its
`inference_sha256` label has no derivation specification in the handoff; it is
not used as a UMI bundle digest or proof of the Git source. The original ZIP's
`SHA256SUMS` and `FILE_LIST.json` enumerate that archive; they are not included in
this source-only Git directory. `MODEL_ARTIFACT.json` binds the separate model
assets here. The asset checker does not verify all source files or establish
rights, accuracy or competition eligibility. Use the community importer's
whole-archive checks when preparing an evaluator bundle.

Third-party terms and modifications are recorded in `PROVENANCE.md` and
`licenses/`. Read them before redistribution.
