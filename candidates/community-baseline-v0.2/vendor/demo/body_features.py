...
class PoseProcessor:
    ...
    def __init__(self, pose_indices: Optional[List[int]]=None, normalize_keypoints: bool=True, fill_missing_value: float=-9999.0):
        """
    ...
    def normalize_pose_keypoints(self, pose_landmarks: List[List[float]]) -> List[List[float]]:
        ...
        translation_matrix = np.array([[1, 0, -signing_space_left], [0, 1, -signing_space_top], [0, 0, 1]])
        scale_matrix = np.array([[1 / signing_space_width, 0, 0], [0, 1 / signing_space_height, 0], [0, 0, 1]])
        shift_matrix = np.array([[1, 0, -0.5], [0, 1, -0.5], [0, 0, 1]])
    ...
    def process_and_save_landmarks(self, landmarks_data: Dict[int, Any], output_path: str, filename: str) -> str:
        """
...
def process_pose_landmarks(landmarks_data: Dict[int, Any], normalize: bool=True, pose_indices: Optional[List[int]]=None) -> np.ndarray:
    """
...
def main():
    ...
    parser.add_argument('--files_list', type=str, required=True, help='path to the pose file')
    parser.add_argument('--pose_features_path', type=str, required=True, help='path to the pose features file')
