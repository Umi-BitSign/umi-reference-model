...
class HandExtractor:
    ...
    def __init__(self, output_size: Tuple[int, int]=(224, 224), scale_factor: float=1.5, distance_threshold: float=0.1):
        """
    ...
    def get_bounding_box(self, landmarks: List[List[float]], image_shape: Tuple[int, int, int], scale_factor: float=1.2) -> Tuple[int, int, int, int]:
    ...
    def adjust_bounding_box(self, bounding_box: Tuple[int, int, int, int], image_shape: Tuple[int, int, int]) -> Tuple[int, int, int, int]:
    ...
    def select_hands(self, pose_landmarks: List[List[float]], hand_landmarks: Optional[List[List[List[float]]]], image_shape: Tuple[int, int, int]) -> Tuple[Optional[List[List[float]]], Optional[List[List[float]]]]:
        """
        ...
        for i in range(len(video)):
            ...
            if frame_landmarks.get('pose_landmarks') is None:
                ...
                if prev_right_frame is not None:
                    ...
                    right_hand_frames.append(np.zeros((*self.output_size, 3), dtype=np.uint8))
                continue
            left_hand_landmarks, right_hand_landmarks = self.select_hands(frame_landmarks['pose_landmarks'][0], frame_landmarks.get('hand_landmarks'), frame_rgb.shape)
    ...
    def extract_and_save_hand_videos(self, video_input, landmarks_data: Dict[int, Any], output_dir: str, video_name: Optional[str]=None) -> Tuple[str, str]:
        """
...
def extract_hand_frames(video_input, landmarks_data: Dict[int, Any], output_size: Tuple[int, int]=(224, 224)) -> Tuple[List[np.ndarray], List[np.ndarray]]:
    """
...
def main():
    ...
    parser.add_argument('--hand_path', type=str, required=True, help='hand path')
