...
class FaceExtractor:
    ...
    def __init__(self, output_size: Tuple[int, int]=(224, 224), scale_factor: float=1.2, grey_background_color: int=128):
        """
    ...
    def calculate_bounding_box(self, landmarks: List[List[float]], indices: List[int], image_shape: Tuple[int, int, int]) -> Tuple[int, int, int, int]:
        ...
        return (int(left * image_shape[1]), int(top * image_shape[0]), int(right * image_shape[1]), int(bottom * image_shape[0]))

    def crop_and_paste(self, src: np.ndarray, dst: np.ndarray, src_box: Tuple[int, int, int, int], dst_origin: Tuple[int, int]):
    ...
    def select_face(self, pose_landmarks: List[List[float]], face_landmarks: List[List[List[float]]]) -> List[List[float]]:
        ...
        distances = [np.linalg.norm(np.array(nose_landmark_from_pose) - np.array(nose_landmark)) for nose_landmark in nose_landmarks_from_face]
    ...
    def extract_face_frames(self, video_input, landmarks_data: Dict[int, Any]) -> List[np.ndarray]:
        ...
        for i in range(len(video)):
            ...
            if frame_landmarks.get('face_landmarks') is not None:
                selected_face = self.select_face(frame_landmarks['pose_landmarks'][0], frame_landmarks['face_landmarks'])
    ...
    def extract_and_save_face_video(self, video_input, landmarks_data: Dict[int, Any], output_dir: str, video_name: Optional[str]=None) -> str:
        """
...
def extract_face_frames(video_input, landmarks_data: Dict[int, Any], output_size: Tuple[int, int]=(224, 224)) -> List[np.ndarray]:
    """
...
def main():
    ...
    parser.add_argument('--face_path', type=str, required=True, help='face path')
